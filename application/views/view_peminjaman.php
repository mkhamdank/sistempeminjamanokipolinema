    ...
    <ul class="nav navbar-nav navbar-right">
      <li><a href=""><b>Halo, <?php if(isset($username)){
        ...
        <?php } else if (ISSET($username)) { ?>
          <li><a href="login_controller/logout">Logout</a></li>
        <?php } ?>
    </ul>
  </div>
</nav>
...
<div class="container">
  <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="background-color: black">
      ...
      <h2 style="color: yellow"><b>Daftar Alat UKM PASTI</b></h2><img src="<?php echo base_url('assets/img/stopnarkoba.png') ?>" width="95px"><br><br>
        <table class="table table-bordered text-center" style="color: yellow">
          <thead>
            <tr>
              <td><b>Nama Alat</b></td>
              <td><b>Jumlah Alat</b></td>
              <td><b>Aksi</b></td>
            </tr>
          </thead>
          <tbody>
          <?php foreach ($array_alat as $key) {?>
            <tr>
              <td><?php echo $key['namaalat']?></td>
              <td><?php echo $key['jumlahalat']?></td>
              <td><a href="<?php echo site_url('peminjaman_controller/edit_dataalat/'.$key['kodealat']);?>" class="btn btn-default">Edit</a> <a href="<?php echo site_url('peminjaman_controller/delete_dataalat/'.$key['kodealat']);?>" class="btn btn-default">Delete</a>
              <a href="<?php echo site_url('peminjaman_controller/pinjam/'.$key['kodealat']);?>" class="btn btn-default">Pinjam</a></td>
            </tr>
            <?php } ?>
          </tbody>
        </table>
        <a href="<?php echo site_url();?>/peminjaman_controller/create_dataalat" type="button" class="btn btn-default">Tambah Daftar Alat</a>
